                      ...
                      TfToken const   &reprToken) override
    {
        ...
        if (_smoothNormals && !_normalsValid) {
            ...
            if (_usingRefinedTopology &&
                ...
                if (!_ComputeSubdivisionLimitNormals(
                        coarseNormals, &_computedNormals)) {
...
private:
    ...
    PxOsdTopologyRefinerSharedPtr _CreateLimitNormalsTopologyRefiner() const
    {
        _HdEmscriptenScopedTimer timer(
            std::string("_CreateLimitNormalsTopologyRefiner ") +
            GetId().GetString());
        ...
        if (_topology.GetScheme() == PxOsdOpenSubdivTokens->loop &&
            !_IsLoopSubdivisionTopologyValid(_topology)) {
            TF_WARN("Cannot compute loop subdivision limit normals for <%s> "
                "due to non-triangular faces.", GetId().GetText());
            return nullptr;
        }
    ...
    bool _ComputeSubdivisionLimitNormals(
        VtVec3fArray const &fallbackNormals,
        VtVec3fArray *normals) const
    {
        _HdEmscriptenScopedTimer timer(
            std::string("_ComputeSubdivisionLimitNormals ") +
            GetId().GetString());

        if (_points.empty() || !normals) {
            return false;
        }

        PxOsdTopologyRefinerSharedPtr refiner =
            _CreateLimitNormalsTopologyRefiner();
        if (!refiner) {
            return false;
        }
        ...
        OpenSubdiv::Far::TopologyLevel const &baseLevel =
            refiner->GetLevel(0);
        const int coarseVertexCount = baseLevel.GetNumVertices();
        ...
        const int regularFaceSize =
            OpenSubdiv::Sdc::SchemeTypeTraits::GetRegularFaceSize(
                refiner->GetSchemeType());

        std::vector<char> assigned(coarseVertexCount, false);
        std::vector<_LimitNormalLocation> limitLocations;
        limitLocations.reserve(coarseVertexCount);

        static const float quadS[4] = {0.0f, 1.0f, 1.0f, 0.0f};
        static const float quadT[4] = {0.0f, 0.0f, 1.0f, 1.0f};
        static const float triS[3] = {0.0f, 1.0f, 0.0f};
        static const float triT[3] = {0.0f, 0.0f, 1.0f};

        const int faceCount = baseLevel.GetNumFaces();
        for (int face = 0; face < faceCount; ++face) {
            OpenSubdiv::Far::ConstIndexArray faceVertices =
                baseLevel.GetFaceVertices(face);
            const int faceVertexCount = faceVertices.size();
            if (faceVertexCount <= 0) {
                continue;
            }

            const int basePtexIndex = ptexIndices.GetFaceId(face);
            const bool isRegularFace = faceVertexCount == regularFaceSize;

            for (int localVertex = 0;
                 localVertex < faceVertexCount;
                 ++localVertex) {
                const int vertexIndex = faceVertices[localVertex];
                if (vertexIndex < 0 ||
                    vertexIndex >= coarseVertexCount ||
                    assigned[vertexIndex]) {
                    continue;
                }

                _LimitNormalLocation location;
                location.vertexIndex = vertexIndex;
                if (isRegularFace) {
                    location.ptexIndex = basePtexIndex;
                    if (faceVertexCount == 4) {
                        location.s = quadS[localVertex];
                        location.t = quadT[localVertex];
                    } else if (faceVertexCount == 3) {
                        location.s = triS[localVertex];
                        location.t = triT[localVertex];
                    } else {
                        continue;
                    }
                } else {
                    location.ptexIndex = basePtexIndex + localVertex;
                    location.s = 0.0f;
                    location.t = 0.0f;
                }

                assigned[vertexIndex] = true;
                limitLocations.push_back(location);
            }
        }

        if (limitLocations.empty()) {
            return false;
        }

        using LocationArray =
            OpenSubdiv::Far::LimitStencilTableFactory::LocationArray;
        std::vector<float> sValues(limitLocations.size());
        std::vector<float> tValues(limitLocations.size());
        OpenSubdiv::Far::LimitStencilTableFactory::LocationArrayVec
            locationArrays;
        locationArrays.reserve(limitLocations.size());
        for (size_t i = 0; i < limitLocations.size(); ++i) {
            sValues[i] = limitLocations[i].s;
            tValues[i] = limitLocations[i].t;

            LocationArray locationArray;
            locationArray.ptexIdx = limitLocations[i].ptexIndex;
            locationArray.numLocations = 1;
            locationArray.s = &sValues[i];
            locationArray.t = &tValues[i];
            locationArrays.push_back(locationArray);
        }

        OpenSubdiv::Far::LimitStencilTableFactory::Options options;
        options.interpolationMode =
            OpenSubdiv::Far::LimitStencilTableFactory::INTERPOLATE_VERTEX;
        options.generate1stDerivatives = true;

        std::unique_ptr<OpenSubdiv::Far::LimitStencilTable const> stencils(
            OpenSubdiv::Far::LimitStencilTableFactory::Create(
                *refiner, locationArrays, nullptr, nullptr, options));
        if (!stencils) {
            return false;
        }

        std::vector<_OsdPrimvar<GfVec3f>> controlPoints(coarseVertexCount);
        for (int i = 0; i < coarseVertexCount; ++i) {
            controlPoints[i].value = _points[i];
        }

        std::vector<_OsdPrimvar<GfVec3f>> du(limitLocations.size());
        std::vector<_OsdPrimvar<GfVec3f>> dv(limitLocations.size());
        stencils->UpdateDerivs(controlPoints, du, dv);

        *normals = fallbackNormals;
        if (normals->size() < _points.size()) {
            normals->resize(_points.size(), GfVec3f(0.0f));
        }

        for (size_t i = 0; i < limitLocations.size(); ++i) {
            GfVec3f normal = GfCross(du[i].value, dv[i].value);
            if (normal.Normalize() == 0.0f) {
                continue;
            }

            const int vertexIndex = limitLocations[i].vertexIndex;
            if (static_cast<size_t>(vertexIndex) < fallbackNormals.size() &&
                GfDot(normal, fallbackNormals[vertexIndex]) < 0.0f) {
                normal = -normal;
            }
